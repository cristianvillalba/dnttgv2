...
void main() {
    ...
    float n_phi = 0.5;
    //float p_phi = 0.3;
	vec4 cval = texelFetch(p3d_Texture0, ivec2(texcoord*params.z), 0);
	vec4 nval = texelFetch(p3d_Texture1, ivec2(texcoord*params.z), 0);
	//vec4 pval = texture(iChannel2, ivec2(texcoord*params.z), 0);
    ...
    for(int i=0; i<25; i++)
    {
        ...
        vec4 ctmp = texelFetch(p3d_Texture0, ivec2(uv), 0);
        vec4 t = cval - ctmp;
        float dist2 = dot(t,t);
        float c_w = min(exp(-(dist2)/c_phi), 1.0);
        
        vec4 ntmp = texelFetch(p3d_Texture1, ivec2(uv), 0);
        t = nval - ntmp;
        dist2 = max(dot(t,t), 0.0);
        float n_w = min(exp(-(dist2)/n_phi), 1.0);
        
        //vec4 ptmp = texture(iChannel2, vec2(uv/params.z), 0);
        //t = pval - ptmp;
        //dist2 = dot(t,t);
