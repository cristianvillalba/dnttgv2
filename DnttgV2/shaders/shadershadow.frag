...
vec3 directLight; //check if mipmaps are working correctly
...
void getRayShadow(vec3 ro, vec3 rd, out float alpha)
{
	vec3 rayOrigin = ro;
	...
	for (int i = 4; i >= 0; i--) {
		...
		if (hit){ //if hit then
			...
			{
				alpha = 1.0; //make this pixel visible
				
				//check shadow casting ray from fragment into light source
...
void main() {
	...
	getRayShadow(ro, rd, alpha); //advance mipmap
